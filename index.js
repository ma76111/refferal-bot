...
import { mainMenu, adminMenu, adminPanelKeyboard, userLanguageKeyboard, getMainMenuKeyboard, depositReviewKeyboard } from './utils/keyboards.js';
...
// const adminStates = new Map(); // ??????? adminStates ?? adminHandler
  ...
  try {
    ...
    if (user && user.is_banned) {
      const bannedMessages = {
        ar: '🚫 عذراً، تم حظرك من استخدام البوت\n\nللاستفسار تواصل مع الدعم',
        en: '🚫 Sorry, you have been banned from using the bot\n\nFor inquiries, contact support',
        ru: '🚫 Извините, вы заблокированы от использования бота\n\nДля запросов свяжитесь с поддержкой'
      };
      await bot.sendMessage(chatId, bannedMessages[userLang]);
    ...
    const isAdmin = config.ADMIN_IDS.includes(telegramId);
    const menu = getMainMenuKeyboard(isAdmin, userLang);
...
bot.on('callback_query', async (query) => {
    ...
    for (const submission of submissions) {
      ...
      const keyboard = submission.status === 'pending' ? {
        reply_markup: {
          inline_keyboard: [
            [
              { text: '✅ قبول', callback_data: `review_accept_${submission.id}` },
              { text: '❌ رفض', callback_data: `review_reject_${submission.id}` }
            ]
          ]
        }
      } : {};
  ...
  if (data === 'back_to_menu') {
    ...
    await bot.sendMessage(query.message.chat.id, '📋 القائمة الرئيسية:', isAdmin ? adminMenu : mainMenu);
    await bot.answerCallbackQuery(query.id);
    return;
  }
});
